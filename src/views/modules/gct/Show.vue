<template>
  <div v-if="loaded">
    <h3>
      <input class="editable" v-model="gct.name" @change="change()" placeholder="请输入名称..." />
    </h3>

    <div class="mt-3" v-show="gct.server">
      <!-- btn group -->
      <div class="btn-group" role="group" aria-label="控制面板和电源管理">
        <a v-if="gct.server" type="button" class="btn btn-outline-primary" target="_blank"
          :href="'https://panel.muhanyun.cn/server/' + gct.server.identifier">前往控制面板</a>

        <button type="button" class="btn btn-outline-primary" @click="setState('start')">
          开机
        </button>
        <button type="button" class="btn btn-outline-primary" @click="setState('restart')">
          重启
        </button>
        <button type="button" class="btn btn-outline-warning" @click="setState('stop')">
          停止
        </button>
      </div>
    </div>
    ...
    <div style="height: 100%; background: #002833" class="mt-3">
      ...
      <input class="command" type="text" v-model="command" placeholder="在这里输入命令..." style="height: 20px"
    ...
    <div v-show="gct.server">
      ...
      <div class="mt-3">
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
          ...
          <li class="nav-item" role="presentation">
            <button class="nav-link" id="pills-feature-tab" data-bs-toggle="pill" data-bs-target="#pills-feature"
        ...
        <div class="tab-content" id="pills-tabContent">
          <div class="tab-pane fade show active" id="pills-basic" role="tabpanel" aria-labelledby="pills-basic-tab">
            <div class="mt-3">
              ...
              <input type="range" class="form-range" min="100" max="800" step="100" v-model="gct.cpu_limit" />
              将使用 <span v-text="gct.cpu_limit / 100"></span> 个核心
            </div>

            <div class="mt-3">
              <h4 class="mt-5">您想要多大内存？</h4>
              <input type="range" class="form-range" min="1024" max="10240" step="512" v-model="gct.memory" />
              将使用 <span v-text="gct.memory / 1024"></span> GB 内存
            </div>

            <div class="mt-3">
              <h4 class="mt-5">您需要多大的磁盘？</h4>
              <input type="range" class="form-range" min="512" max="40960" step="512" v-model="gct.disk" />
          ...
          <div class="tab-pane fade" id="pills-feature" role="tabpanel" aria-labelledby="pills-feature-tab">
            <div class="mt-3">
              ...
              <input type="range" class="form-range" min="1" max="20" v-model="gct.backups" />
              将可以创建 <span v-text="gct.backups"></span> 个备份
              <small>修改此项目时，您应该确保从控制面板中删除了多余的备份。</small>
            </div>

            <div class="mt-3">
              <h4 class="mt-5">您需要多少端口？</h4>
              <input type="range" class="form-range" min="1" max="5" v-model="gct.allocations" />
              将分配 <span v-text="gct.allocations"></span> 个端口
              <small>修改此项目时，您应该确保从控制面板中删除了多余的端口。</small>
            </div>

            <div class="mt-3">
              <h4 class="mt-5">要多少个数据库？</h4>
              <input type="range" class="form-range" min="0" max="5" v-model="gct.databases" />
              将分配 <span v-text="gct.databases"></span> 个数据库
              <small>修改此项目时，您应该确保从控制面板中删除了多余的数据库。</small>
            </div>
          </div>
          <div class="tab-pane fade" id="pills-template" role="tabpanel" aria-labelledby="pills-template-tab">
            ...
            <div class="row">
              <div class="col">
                <select class="form-select form-select-sm" v-model="selected_nest_id" @change="updateEggs()">
              ...
              <div class="col" v-show="selected_nest_id">
                <select class="form-select form-select-sm" v-model="gct.egg_id" @change="updateThisEgg()">
