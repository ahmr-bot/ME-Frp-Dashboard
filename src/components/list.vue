 ...
 * @LastEditTime: 2022-11-24 07:49:02
...
<template>
  <v-list>
    <v-list-item :prepend-avatar=avatar>
      <v-list-item-tittle>{{ store.state.user.name }}</v-list-item-tittle>
    </v-list-item>
  </v-list>
  <v-list>
  ...
  <v-divider></v-divider>
  <v-list>
    <v-list-item prepend-icon="list" title="隧道相关" :to="{ name: 'modules.tunnels' }"></v-list-item>
  </v-list>
  <v-list>
    <v-list-item prepend-icon="download" title="软件下载" :to="{ name: 'modules.tunnels.download' }"></v-list-item>
  </v-list>
  <v-list>
    <v-list-item prepend-icon="description" title="配置文件" :to="{ name: 'modules.tunnels.compact' }"></v-list-item>
  </v-list>
  <v-list>
    <v-list-item prepend-icon="cloud_done" title="服务状态" :to="{ name: 'servers' }"></v-list-item>
  </v-list>
  <v-divider></v-divider>
  <v-list>
...
const avatar = 'https://gravatar.loli.net/avatar/' + md5(store.state.user.email) + '?s=64'
console.log(avatar)
</script>